  ...
  if (v != 0 && rsize < (size_t)(m->dvsize - nb)) {
    if (RTCHECK(ok_address(m, v))) { /* split */
      ...
      if (RTCHECK(ok_next(v, r))) {
        ...
        if (rsize < MIN_CHUNK_SIZE){
		  ...
		  mchunkptr p  = (mchunkptr) v;
			...
			asm volatile (
				...
				:
				: [bitmap_idx]  "r" (bitmap_idx(curr_ptr)),
				  [bitmap_addr] "r" (bitmap_addr(curr_ptr)),
				  [area_sz]     "Ir" (area_sz()));
			asm volatile (
				"setc %[bit]\n\t"
				: [bit] "+rm" (is_written));
			...
			{
				u64 shadow_val;
				// ---------- sm_assert_8b(curr_ptr) for current chunk size
				asm volatile (
					"mov %%gs:0x0(%[s]), %[val]\n\t"
					: [val] "=r" (shadow_val)
					: [s] "r" (curr_ptr)
				...
				if(shadow_val != *curr_ptr) {
					fprintf(stderr, " [errno: %d] %s\n", errno, "secure memory value mismatched\n");
					exit(EXIT_FAILURE);
				}

				// ---------- sm_assert_8b(next_ptr) for next chunk
				asm volatile (
					"mov %%gs:0x0(%[s]), %[val]\n\t"
					: [val] "=r" (shadow_val)
					: [s] "r" (next_ptr)
		...
		curr_ptr = ((u64*) v) + 1;
		next_ptr = (u64*) chunk_plus_offset(r, rsize);
		next_ptr_size = next_ptr + 1;	
	
		// ---------- sm_write_8b(curr_ptr) for current chunk size.
		unsigned long hyperspace_addr = ((unsigned long) HYPR_START) + ((unsigned long) curr_ptr);
		*((unsigned long*)hyperspace_addr) = *((unsigned long*) curr_ptr);

		hyperspace_addr = ((unsigned long) HYPR_START) + ((unsigned long) next_ptr);	
		*((unsigned long*)hyperspace_addr) = *((unsigned long*) next_ptr);

		hyperspace_addr = ((unsigned long) HYPR_START) + ((unsigned long) next_ptr_size);	
		*((unsigned long*)hyperspace_addr) = *((unsigned long*) next_ptr_size);

		asm volatile (
			"btsq %[bitmap_idx_0_curr_ptr], %%gs:(%[bitmap_addr_curr_ptr],%[area_sz])\n\t" // for current chunk size
			"btsq %[bitmap_idx_1_curr_ptr], %%gs:(%[bitmap_addr_curr_ptr],%[area_sz])\n\t"
			"btsq %[bitmap_idx_0_next_ptr], %%gs:(%[bitmap_addr_next_ptr],%[area_sz])\n\t" // for next chunk prev_size
			"btsq %[bitmap_idx_1_next_ptr], %%gs:(%[bitmap_addr_next_ptr],%[area_sz])\n\t"
			"btsq %[bitmap_idx_0_next_ptr_size], %%gs:(%[bitmap_addr_next_ptr_size],%[area_sz])\n\t" // for next chunk size
			"btsq %[bitmap_idx_1_next_ptr_size], %%gs:(%[bitmap_addr_next_ptr_size],%[area_sz])\n\t"
			:
			: [bitmap_idx_0_curr_ptr]  	"r" (bitmap_idx(curr_ptr)),
			  [bitmap_idx_1_curr_ptr]  	"r" (bitmap_idx(curr_ptr)+1),
			  [bitmap_addr_curr_ptr] 	"r" (bitmap_addr(curr_ptr)),
			  [bitmap_idx_0_next_ptr]  	"r" (bitmap_idx(next_ptr)),
			  [bitmap_idx_1_next_ptr]  	"r" (bitmap_idx(next_ptr)+1),
			  [bitmap_addr_next_ptr] 	"r" (bitmap_addr(next_ptr)),
			  [bitmap_idx_0_next_ptr_size]  	"r" (bitmap_idx(next_ptr_size)),
			  [bitmap_idx_1_next_ptr_size]  	"r" (bitmap_idx(next_ptr_size)+1),
			  [bitmap_addr_next_ptr_size] 		"r" (bitmap_addr(next_ptr_size)),
			  [area_sz]     			"Ir" (area_sz())
			: "memory"
		);
...
void* mspace_malloc(mspace msp, size_t bytes) {
  ...
  postaction:
	...
	if (!written) {
		...
		mchunkptr p = mem2chunk(mem);
		curr_ptr = ((u64*) p) + 1;
		next_ptr = (u64*) chunk_plus_offset(p, chunksize(p));
		next_ptr_size = next_ptr + 1;
...
void mspace_free(mspace msp, void* mem) {
  if (mem != 0) {
    mchunkptr p  = mem2chunk(mem);
	//----OTWM-Assert-Start--
	size_t p_size = chunksize(p);
	mchunkptr r = chunk_plus_offset(p, p_size);
	u64* p1 = ((u64*) p) + 1;
	...
	asm volatile (
		"btq %[bitmap_idx], %%gs:(%[bitmap_addr],%[area_sz])\n\t"
		:
		: [bitmap_idx]  "r" (bitmap_idx((u64*) p)),
		  [bitmap_addr] "r" (bitmap_addr((u64*) p)),
		  [area_sz]     "Ir" (area_sz())
	);
	asm volatile (
		"setc %[bit]\n\t"
	...
	if(is_written)
	{
		//otwm_assert(curr_ptr);
		u64 shadow_val;
		// ---------- sm_assert_8b(curr_ptr)
		asm volatile (
			"mov %%gs:0x0(%[s]), %[val]\n\t"
			: [val] "=r" (shadow_val)
			: [s] "r" ((u64*) p)
		...
		// ---------- sm_assert_8b(prev_size of next check)
		asm volatile (
			"mov %%gs:0x0(%[s]), %[val]\n\t"
			: [val] "=r" (shadow_val)
			: [s] "r" ((u64*) r)
