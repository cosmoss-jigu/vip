...
bool free_fastpath(void *ptr, size_t size, bool size_hint) {
	...
	if (config_stats) {
		...
		*tsd_thread_deallocatedp_get(tsd) += usize;
		// OTWM) sm_deregister --------------------------------------------------------- 
		//XXX:This is executed everytime regardless of sensitivity of pointer. 
...
#define SLOT32_MASK       ~0xFF
		...
		aligned_start = (void*)(((u64)ptr + (SLOT32_SZ - 1)) & SLOT32_MASK);
		end = ptr + usize;
		...
		if (pkru == 0x8) {
			asm volatile (
				".byte 0x0f, 0x01, 0xef"
				: // No Outputs
				: "a" (0), "c" (0), "d" (0) // "a" is (rights << 2)
			);
		}

		if (aligned_size != 0) {
			void *hyperspace = ((void *)bitmap_addr(ptr)) + (2 * area_sz());
		...
		if (aligned_start != ptr) {
			...
			for (; addr < aligned_start; addr++) {
				...
				asm volatile (
					"btrq %[bitmap_idx1], %%gs:(%[bitmap_addr],%[area_sz])\n\t"
					"btrq %[bitmap_idx2], %%gs:(%[bitmap_addr],%[area_sz])\n\t"
					:
					: [bitmap_idx1]  "r" (bitmap_idx(addr)),
					  [bitmap_idx2]  "r" (bitmap_idx(addr)+1),
